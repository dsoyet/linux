...
local config = {
    ...
    -- ── Auto VR format detection ──
    auto_detect = true,                 -- Enable filename + resolution auto-detection
    fisheye_keywords = "fisheye,FS,fish,190,200,220,VR190,VR200,VR220",
    vr180_keywords   = "180,SBS,VR180,3DV,HSBS,HALF_SBS,OU,HOU,HALF_OU",
    -- Resolution ratio range that suggests VR (e.g. 3840x1920 = 2.0)
    ratio_2_1_min = 1.90,
    ratio_2_1_max = 2.10,
    -- What to do when ratio is ~2:1 but no keyword matched
    -- "vr180" = force VR180 (default)  |  "prompt" = show hint  |  "none" = ignore
    unknown_2to1_action = "vr180",
    auto_detect_osd = 3,                -- OSD duration for detection messages (seconds)
}
...
-- ── Auto VR detection: keyword matcher ──

local last_auto_detected = nil

local function match_keywords(filename, keyword_list)
    if not keyword_list or keyword_list == "" then return false end
    local upper = filename:upper()
    for kw in keyword_list:gmatch("[^,]+") do
        local trimmed = kw:match("^%s*(.-)%s*$"):upper()
        if trimmed == "" then goto continue end
        local escaped = trimmed:gsub("([%^%$%(%)%%%.%[%]%*%+%-%?])", "%%%1")
        -- Whole-word match (boundaries are non-letter chars or string edges)
        if upper:find("(^|[^%a])" .. escaped .. "([^%a]|$)") then
            mp.msg.info("mpv360 auto-detect: matched keyword '" .. trimmed .. "' (word boundary)")
            return true
        end
        -- Fallback: plain substring for keywords >= 5 chars (catches FISHEYE190 etc.)
        if #trimmed >= 5 and upper:find(trimmed, 1, true) then
            mp.msg.info("mpv360 auto-detect: matched keyword '" .. trimmed .. "' (substring fallback)")
            return true
        end
        ::continue::
    end
...
-- Allow external scripts to call commands via script-message or script-binding
mp.register_script_message(function(cmd)
    local fn = commands[cmd]
    if fn then
        fn()
        if cmd ~= "show-help" and cmd ~= "toggle" then
            update_params()
        end
    else
        mp.msg.warn("mpv360: unknown command '" .. cmd .. "'")
    end
end)

-- Smart F19: auto-enable 360 mode + toggle mouse look
mp.add_key_binding("", "smart-f19", function()
    mp.msg.warn("mpv360: smart-f19 triggered, enabled=" .. tostring(config.enabled) .. " mouse_look=" .. tostring(mouse_look_active))
    if not config.enabled then
        mp.osd_message("mpv360: enabling 360 mode...", 1)
        enable()
        mp.add_timeout(0.15, function()
            mp.msg.warn("mpv360: timeout fired, enabled=" .. tostring(config.enabled))
            if config.enabled then
                mp.osd_message("mpv360: entering mouse look...", 1)
                start_mouse_look()
            end
        end)
    elseif mouse_look_active then
        mp.osd_message("mpv360: exiting mouse look", 1)
        stop_mouse_look()
    else
        mp.osd_message("mpv360: entering mouse look (already enabled)", 1)
        start_mouse_look()
    end
end)

-- ── Auto VR format detection on file load ──
local function auto_detect_and_apply()
    if not config.auto_detect then return end
    ...
    mp.msg.info(string.format("mpv360 auto-detect: file='%s' %dx%d ratio=%.2f", filename, width, height, ratio))

    local detected = nil
    local detail = ""

    -- L1: filename keyword match (fisheye > vr180)
    if match_keywords(filename, config.fisheye_keywords) then
        detected = "fisheye190"
        detail = "keyword (fisheye)"
    elseif match_keywords(filename, config.vr180_keywords) then
        detected = "vr180"
        detail = "keyword (VR180)"
    end

    -- L2: resolution ratio fallback — ~2:1 without keywords = VR180
    if not detected and ratio >= config.ratio_2_1_min and ratio <= config.ratio_2_1_max then
        if config.unknown_2to1_action == "prompt" then
            mp.osd_message(string.format(
                "⚠ 2:1 比例视频 (%.1f:1)，可能是 VR 内容\nCtrl+Shift+P 切换投影 | Ctrl+E 开关 360 模式",
                ratio
            ), config.auto_detect_osd)
            mp.msg.info("mpv360 auto-detect: 2:1 ratio — prompting user")
        elseif config.unknown_2to1_action ~= "none" then
            detected = "vr180"
            detail = string.format("2:1 ratio (%.2f)", ratio)
        end
    end

    -- Apply
    mp.msg.info(string.format("mpv360 auto-detect: result='%s' last='%s'", tostring(detected), tostring(last_auto_detected)))
    if detected and detected ~= last_auto_detected then
        if detected == "fisheye190" then
            config.input_projection = 1
            config.fisheye_fov = math.rad(190)
            config.eye = 0
            if not config.enabled then enable() else update_params() end
            mp.osd_message("🐟 VR 鱼眼 190° (" .. detail .. ")", config.auto_detect_osd)
        elseif detected == "vr180" then
            config.input_projection = 2
            config.fisheye_fov = math.rad(180)
            config.eye = 0
            if not config.enabled then enable() else update_params() end
            mp.osd_message("🥽 VR 180° SBS (" .. detail .. ")", config.auto_detect_osd)
        end
        mp.msg.info("mpv360 auto-detect: '" .. detected .. "' (" .. detail .. ")")
    elseif not detected and last_auto_detected then
        if config.enabled then disable() end
        mp.osd_message("📺 2D 模式", config.auto_detect_osd)
        mp.msg.info("mpv360 auto-detect: disabled (2D)")
    end

    last_auto_detected = detected
end

mp.register_event("file-loaded", function()
    mp.add_timeout(0.3, auto_detect_and_apply)
end)

-- Legacy: if manually set enabled=true in config, enable at startup
if config.enabled then
    enable()
end
